...
function update_win() {
    ...
    var recode = grid_panel.getSelectionModel().getSelected();
    var interfaceNumber = recode.get("interfaceNumber");
    alert(interfaceNumber);
    var tableNameEn = recode.get("tableNameEn");
    var formPanel = new Ext.form.FormPanel({
        autoScroll: true,
        //layout: 'column',
        height: 100,
    ...
    var store = new Ext.data.GroupingStore({
        ...
        proxy:new Ext.data.HttpProxy({
            url: "../../InterfaceAction_findColumnStore.action?interfaceNumber=" + escape(interfaceNumber)+"&table_name='"+escape(tableNameEn)+"'"
        })
    });
    ...
    var grid = new Ext.grid.EditorGridPanel({
        ...
        listeners: {
            ...
            var current_grid = this;
            store.on('load', function(){
                var data = current_grid.getStore().data.dataRows;
                var recs = [];
                ...
                current_grid.getSelectionModel().selectRows(recs);
            })
            }
        }
    });
